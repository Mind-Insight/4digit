...
class VideoPersonalityDataset(Dataset):
    def __init__(self, video_dir, parsed_data, device):
    ...
    def extract_video_features(self, video_path):
        ...
        return video_tensor.mean(dim=0).unsqueeze(0).to(self.device)
...
class PersonalityModel(pl.LightningModule):
    ...
    def forward(self, video_features, text_features):
        ...
        text_outputs = torch.zeros(video_outputs.size(0), 512).to(device)
        combined = torch.cat((video_outputs, text_outputs), dim=1).to(device)
...
if __name__ == "__main__":
    ...
    dataset = VideoPersonalityDataset(VIDEO_DIR, parse, device)
